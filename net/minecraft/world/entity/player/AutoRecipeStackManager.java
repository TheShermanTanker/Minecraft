...
public class AutoRecipeStackManager {
    ...
    public void accountSimpleStack(ItemStack stack) {
        if (!stack.isDamaged() && !stack.hasEnchantments() && !stack.hasName()) {
    ...
    public boolean canCraft(IRecipe<?> recipe, @Nullable IntList output) {
        return this.canCraft(recipe, output, 1);
    }

    public boolean canCraft(IRecipe<?> recipe, @Nullable IntList output, int multiplier) {
        return (new AutoRecipeStackManager.RecipePicker(recipe)).tryPick(multiplier, output);
    }

    public int getBiggestCraftableStack(IRecipe<?> recipe, @Nullable IntList output) {
        return this.getBiggestCraftableStack(recipe, Integer.MAX_VALUE, output);
    }

    public int getBiggestCraftableStack(IRecipe<?> recipe, int limit, @Nullable IntList output) {
        return (new AutoRecipeStackManager.RecipePicker(recipe)).tryPickAll(limit, output);
    }

    public static ItemStack fromStackingIndex(int itemId) {
        return itemId == 0 ? ItemStack.EMPTY : new ItemStack(Item.getById(itemId));
    }
    ...
    class RecipePicker {
        ...
        public boolean tryPick(int multiplier, @Nullable IntList output) {
            if (multiplier <= 0) {
                ...
                for(i = 0; this.dfs(multiplier); ++i) {
                    AutoRecipeStackManager.this.take(this.items[this.path.getInt(0)], multiplier);
                    ...
                    if (bl2 && list.get(m).isEmpty()) {
                        ...
                        for(int n = 0; n < this.itemCount; ++n) {
                            if (this.hasResidual(false, l, n)) {
                                ...
                                AutoRecipeStackManager.this.put(this.items[n], multiplier);
                                if (bl2) {
        ...
        private boolean dfs(int multiplier) {
            ...
            for(int j = 0; j < i; ++j) {
                if (AutoRecipeStackManager.this.contents.get(this.items[j]) >= multiplier) {
                    this.visit(false, j);
        ...
        private int getMinIngredientCount() {
            ...
            for(RecipeItemStack ingredient : this.ingredients) {
                ...
                for(int k : ingredient.getStackingIds()) {
                    j = Math.max(j, AutoRecipeStackManager.this.contents.get(k));
                }

...
public class EntityPiglinBrute extends EntityPiglinAbstract {
    ...
    public EntityPiglinBrute(EntityTypes<? extends EntityPiglinBrute> type, World world) {
        super(type, world);
        this.xpReward = 20;
    }

    public static AttributeProvider.Builder createAttributes() {
        return EntityMonster.createMonsterAttributes().add(GenericAttributes.MAX_HEALTH, 50.0D).add(GenericAttributes.MOVEMENT_SPEED, (double)0.35F).add(GenericAttributes.ATTACK_DAMAGE, 7.0D);
    }
    ...
    @Override
    protected BehaviorController.Provider<EntityPiglinBrute> brainProvider() {
    ...
    public boolean damageEntity(DamageSource source, float amount) {
        ...
        if (this.level.isClientSide) {
            ...
            if (bl && source.getEntity() instanceof EntityLiving) {
                PiglinBruteAI.wasHurtBy(this, (EntityLiving)source.getEntity());
            }

...
public class BlockActionContextDirectional extends BlockActionContext {
    ...
    public BlockActionContextDirectional(World world, BlockPosition pos, EnumDirection facing, ItemStack stack, EnumDirection side) {
        super(world, (EntityHuman)null, EnumHand.MAIN_HAND, stack, new MovingObjectPositionBlock(Vec3D.atBottomCenterOf(pos), side, pos, false));
        this.direction = facing;
    }

    @Override
    public BlockPosition getClickPosition() {
        return this.getHitResult().getBlockPosition();
    }

    @Override
    public boolean canPlace() {
        return this.getWorld().getType(this.getHitResult().getBlockPosition()).canBeReplaced(this);
    }

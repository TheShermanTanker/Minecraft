...
public class WorldChunkManagerOverworld extends WorldChunkManager {
    public static final Codec<WorldChunkManagerOverworld> CODEC = RecordCodecBuilder.create((instance) -> {
        ...
        }), Codec.BOOL.optionalFieldOf("legacy_biome_init_layer", Boolean.valueOf(false), Lifecycle.stable()).forGetter((overworldBiomeSource) -> {
            return overworldBiomeSource.legacyBiomeInitLayer;
        }), Codec.BOOL.fieldOf("large_biomes").orElse(false).stable().forGetter((overworldBiomeSource) -> {
            return overworldBiomeSource.largeBiomes;
        }), RegistryLookupCodec.create(IRegistry.BIOME_REGISTRY).forGetter((overworldBiomeSource) -> {
            return overworldBiomeSource.biomes;
    ...
    public WorldChunkManagerOverworld(long seed, boolean legacyBiomeInitLayer, boolean largeBiomes, IRegistry<BiomeBase> biomeRegistry) {
        super(POSSIBLE_BIOMES.stream().map((key) -> {
            return () -> {
        ...
        this.noiseBiomeLayer = GenLayers.getDefaultLayer(seed, legacyBiomeInitLayer, largeBiomes ? 6 : 4, 4);
    }
    ...
    public WorldChunkManager withSeed(long seed) {
        return new WorldChunkManagerOverworld(seed, this.legacyBiomeInitLayer, this.largeBiomes, this.biomes);
    }

    @Override
    public BiomeBase getBiome(int biomeX, int biomeY, int biomeZ) {
        return this.noiseBiomeLayer.get(this.biomes, biomeX, biomeZ);
    }
}

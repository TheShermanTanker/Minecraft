...
public abstract class Column {
    ...
    public static Optional<Column> scan(VirtualWorldReadable world, BlockPosition pos, int height, Predicate<IBlockData> canGenerate, Predicate<IBlockData> canReplace) {
        BlockPosition.MutableBlockPosition mutableBlockPos = pos.mutable();
        if (!world.isStateAtPosition(pos, canGenerate)) {
            return Optional.empty();
        } else {
            int i = pos.getY();
            OptionalInt optionalInt = scanDirection(world, height, canGenerate, canReplace, mutableBlockPos, i, EnumDirection.UP);
            OptionalInt optionalInt2 = scanDirection(world, height, canGenerate, canReplace, mutableBlockPos, i, EnumDirection.DOWN);
            return Optional.of(create(optionalInt2, optionalInt));
        }
    }

    private static OptionalInt scanDirection(VirtualWorldReadable world, int height, Predicate<IBlockData> canGenerate, Predicate<IBlockData> canReplace, BlockPosition.MutableBlockPosition mutablePos, int y, EnumDirection direction) {
        mutablePos.setY(y);

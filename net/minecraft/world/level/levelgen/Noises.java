...
public class Noises {
    ...
    private static ResourceKey<NormalNoise$NoiseParameters> createKey(String id) {
        return ResourceKey.create(IRegistry.NOISE_REGISTRY, new MinecraftKey(id));
    }

    public static NoiseGeneratorNormal instantiate(IRegistry<NormalNoise$NoiseParameters> noiseRegistry, PositionalRandomFactory randomDeriver, ResourceKey<NormalNoise$NoiseParameters> noise) {
        return NoiseGeneratorNormal.create(randomDeriver.fromHashOf(noise.location()), noiseRegistry.getOrThrow(noise));
    }
}

...
public abstract class LightEngineGraph {
    ...
    protected LightEngineGraph(int levelCount, int expectedLevelSize, int expectedTotalSize) {
        if (levelCount >= 254) {
    ...
    private void checkEdge(long sourceId, long id, int level, int currentLevel, int pendingLevel, boolean decrease) {
        if (!this.isSource(id)) {
            ...
            if (decrease) {
                ...
                i = MathHelper.clamp(this.getComputedLevel(id, sourceId, level), 0, this.levelCount - 1);
            }
    ...
    protected final void checkNeighbor(long sourceId, long targetId, int level, boolean decrease) {
        ...
        int j = MathHelper.clamp(this.computeLevelFromNeighbor(sourceId, targetId, level), 0, this.levelCount - 1);
        if (decrease) {

        ...
        try {
            if (byteBuf.readUnsignedByte() == 254) {
                ...
                case 1:
                    ...
                    String string2 = String.format("\u00a71\u0000%d\u0000%s\u0000%s\u0000%d\u0000%d", 127, minecraftServer.getVersion(), minecraftServer.getMotd(), minecraftServer.getPlayerCount(), minecraftServer.getMaxPlayers());
                    this.sendFlushAndClose(channelHandlerContext, this.createReply(string2));
                    break;
                default:
                    boolean bl2 = byteBuf.readUnsignedByte() == 1;
                    bl2 = bl2 & byteBuf.readUnsignedByte() == 250;
                    bl2 = bl2 & "MC|PingHost".equals(new String(byteBuf.readBytes(byteBuf.readShort() * 2).array(), StandardCharsets.UTF_16BE));
                    int j = byteBuf.readUnsignedShort();
                    bl2 = bl2 & byteBuf.readUnsignedByte() >= 73;
                    bl2 = bl2 & 3 + byteBuf.readBytes(byteBuf.readShort() * 2).array().length + 4 == j;
                    bl2 = bl2 & byteBuf.readInt() <= 65535;
                    bl2 = bl2 & byteBuf.readableBytes() == 0;
                    if (!bl2) {
                        return;
                    }

                    LOGGER.debug("Ping: (1.6) from {}:{}", inetSocketAddress.getAddress(), inetSocketAddress.getPort());
                    String string3 = String.format("\u00a71\u0000%d\u0000%s\u0000%s\u0000%d\u0000%d", 127, minecraftServer.getVersion(), minecraftServer.getMotd(), minecraftServer.getPlayerCount(), minecraftServer.getMaxPlayers());

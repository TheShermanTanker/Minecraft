...
public class ArgumentParserSelector {
    ...
    public static final DynamicCommandExceptionType ERROR_UNKNOWN_SELECTOR_TYPE = new DynamicCommandExceptionType((selectorType) -> {
        return new ChatMessage("argument.entity.selector.unknown", selectorType);
    });
    ...
    public static final DynamicCommandExceptionType ERROR_EXPECTED_OPTION_VALUE = new DynamicCommandExceptionType((option) -> {
        return new ChatMessage("argument.entity.options.valueless", option);
    });
    public static final BiConsumer<Vec3D, List<? extends Entity>> ORDER_ARBITRARY = (pos, entities) -> {
    };
    public static final BiConsumer<Vec3D, List<? extends Entity>> ORDER_NEAREST = (pos, entities) -> {
        entities.sort((entity1, entity2) -> {
            return Doubles.compare(entity1.distanceToSqr(pos), entity2.distanceToSqr(pos));
        });
    };
    public static final BiConsumer<Vec3D, List<? extends Entity>> ORDER_FURTHEST = (pos, entities) -> {
        entities.sort((entity1, entity2) -> {
            return Doubles.compare(entity2.distanceToSqr(pos), entity1.distanceToSqr(pos));
        });
    };
    public static final BiConsumer<Vec3D, List<? extends Entity>> ORDER_RANDOM = (pos, entities) -> {
    ...
    public EntitySelector getSelector() {
        ...
        if (this.deltaX == null && this.deltaY == null && this.deltaZ == null) {
            if (this.distance.getMax() != null) {
                ...
                aABB2 = new AxisAlignedBB(-d, -d, -d, d + 1.0D, d + 1.0D, d + 1.0D);
            } else {
                aABB2 = null;
            }
        } else {
    ...
    private AxisAlignedBB createAabb(double x, double y, double z) {
        boolean bl = x < 0.0D;
        boolean bl2 = y < 0.0D;
        boolean bl3 = z < 0.0D;
        double d = bl ? x : 0.0D;
    ...
    private Predicate<Entity> createRotationPredicate(CriterionConditionRange angleRange, ToDoubleFunction<Entity> entityToAngle) {
    ...
    protected void parseOptions() throws CommandSyntaxException {
        ...
        while(true) {
            if (this.reader.canRead() && this.reader.peek() != ']') {
                ...
                int i = this.reader.getCursor();
                String string = this.reader.readString();
                PlayerSelector.Modifier modifier = PlayerSelector.get(this, string, i);
                this.reader.skipWhitespace();
                if (!this.reader.canRead() || this.reader.peek() != '=') {
                    this.reader.setCursor(i);
    ...
    public void setDistance(CriterionConditionValue.DoubleRange distance) {
    ...
    public void setLevel(CriterionConditionValue.IntegerRange experienceRange) {
        this.level = experienceRange;
    }
    ...
    public void setRotX(CriterionConditionRange pitchRange) {
    ...
    public void setRotY(CriterionConditionRange yawRange) {
        this.rotY = yawRange;
    }
    ...
    public void setOrder(BiConsumer<Vec3D, List<? extends Entity>> sorter) {
        this.order = sorter;
    }
    ...
    private static void fillSelectorSuggestions(SuggestionsBuilder builder) {
        builder.suggest("@p", new ChatMessage("argument.entity.selector.nearestPlayer"));
        builder.suggest("@a", new ChatMessage("argument.entity.selector.allPlayers"));
        builder.suggest("@r", new ChatMessage("argument.entity.selector.randomPlayer"));
        builder.suggest("@s", new ChatMessage("argument.entity.selector.self"));
        builder.suggest("@e", new ChatMessage("argument.entity.selector.allEntities"));
    }
    ...
    private CompletableFuture<Suggestions> suggestOptionsKeyOrClose(SuggestionsBuilder builder, Consumer<SuggestionsBuilder> consumer) {
        builder.suggest(String.valueOf(']'));
        PlayerSelector.suggestNames(this, builder);
        return builder.buildFuture();
    }
    ...
    public void setHasGamemodeNotEquals(boolean hasNegatedGameMode) {
        this.hasGamemodeNotEquals = hasNegatedGameMode;
    }
    ...
    public void limitToType(EntityTypes<?> entityType) {

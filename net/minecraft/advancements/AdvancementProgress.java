...
public class AdvancementProgress implements Comparable<AdvancementProgress> {
    ...
    public void serializeToNetwork(PacketDataSerializer buf) {
        buf.writeMap(this.criteria, PacketDataSerializer::writeUtf, (bufx, progresses) -> {
            progresses.serializeToNetwork(bufx);
        });
    }

    public static AdvancementProgress fromNetwork(PacketDataSerializer buf) {
        Map<String, CriterionProgress> map = buf.readMap(PacketDataSerializer::readUtf, CriterionProgress::fromNetwork);
    ...
    public Date getFirstProgressDate() {
        ...
        for(CriterionProgress criterionProgress : this.criteria.values()) {
            if (criterionProgress.isDone() && (date == null || criterionProgress.getDate().before(date))) {
    ...
    public static class Serializer implements JsonDeserializer<AdvancementProgress>, JsonSerializer<AdvancementProgress> {
        ...
        public AdvancementProgress deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext jsonDeserializationContext) throws JsonParseException {
            ...
            JsonObject jsonObject2 = ChatDeserializer.getAsJsonObject(jsonObject, "criteria", new JsonObject());
            AdvancementProgress advancementProgress = new AdvancementProgress();

            for(Entry<String, JsonElement> entry : jsonObject2.entrySet()) {
                String string = entry.getKey();
                advancementProgress.criteria.put(string, CriterionProgress.fromJson(ChatDeserializer.convertToString(entry.getValue(), string)));
            }

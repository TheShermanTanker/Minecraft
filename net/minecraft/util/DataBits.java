...
public class DataBits {
    ...
    public DataBits(int elementBits, int size, @Nullable long[] storage) {
        ...
        if (storage != null) {
            if (storage.length != j) {
                throw (RuntimeException)SystemUtils.pauseInIde(new RuntimeException("Invalid length given for storage, got: " + storage.length + " but expected: " + j));
            }

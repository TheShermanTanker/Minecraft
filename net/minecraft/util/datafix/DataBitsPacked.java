...
public class DataBitsPacked {
    ...
    public DataBitsPacked(int unitSize, int length) {
        this(unitSize, length, new long[MathHelper.roundToward(length * unitSize, 64) / 64]);
    }

    public DataBitsPacked(int unitSize, int length, long[] array) {
        Validate.inclusiveBetween(1L, 32L, (long)unitSize);
        this.size = length;
        this.bits = unitSize;
        this.data = array;
        this.mask = (1L << unitSize) - 1L;
        int i = MathHelper.roundToward(length * unitSize, 64) / 64;
        if (array.length != i) {

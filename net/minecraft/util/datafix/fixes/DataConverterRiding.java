...
public class DataConverterRiding extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        Type<?> type2 = schema2.getTypeRaw(DataConverterTypes.ENTITY_TREE);
        Type<?> type3 = schema.getTypeRaw(DataConverterTypes.ENTITY);
    ...
    private <OldEntityTree, NewEntityTree, Entity> TypeRewriteRule cap(Schema inputSchema, Schema outputSchema, Type<OldEntityTree> inputEntityTreeType, Type<NewEntityTree> outputEntityTreeType, Type<Entity> inputEntityType) {
        Type<Pair<String, Pair<Either<OldEntityTree, Unit>, Entity>>> type = DSL.named(DataConverterTypes.ENTITY_TREE.typeName(), DSL.and(DSL.optional(DSL.field("Riding", inputEntityTreeType)), inputEntityType));
        Type<Pair<String, Pair<Either<List<NewEntityTree>, Unit>, Entity>>> type2 = DSL.named(DataConverterTypes.ENTITY_TREE.typeName(), DSL.and(DSL.optional(DSL.field("Passengers", DSL.list(outputEntityTreeType))), inputEntityType));
        Type<?> type3 = inputSchema.getType(DataConverterTypes.ENTITY_TREE);
        Type<?> type4 = outputSchema.getType(DataConverterTypes.ENTITY_TREE);
        if (!Objects.equals(type3, type)) {
            ...
            Type<?> type5 = inputSchema.getType(DataConverterTypes.PLAYER);
            Type<?> type6 = outputSchema.getType(DataConverterTypes.PLAYER);
            return TypeRewriteRule.seq(this.fixTypeEverywhere("EntityRidingToPassengerFix", type, type2, (dynamicOps) -> {
                return (pair) -> {
                    ...
                    while(true) {
                        ...
                        optional = Optional.of(Pair.of(DataConverterTypes.ENTITY_TREE.typeName(), Pair.of(either, pair2.getSecond().getSecond())));

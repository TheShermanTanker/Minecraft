    ...
    protected TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("EntityUUIDFixes", this.getInputSchema().getType(this.typeReference), (typed) -> {
            ...
            for(String string : ABSTRACT_HORSES) {
                typed = this.updateNamedChoice(typed, string, DataConverterEntityUUID::updateAnimalOwner);
            }

            for(String string2 : TAMEABLE_ANIMALS) {
                typed = this.updateNamedChoice(typed, string2, DataConverterEntityUUID::updateAnimalOwner);
            }

            for(String string3 : ANIMALS) {
                typed = this.updateNamedChoice(typed, string3, DataConverterEntityUUID::updateAnimal);
            }

            for(String string4 : MOBS) {
                typed = this.updateNamedChoice(typed, string4, DataConverterEntityUUID::updateMob);
            }

            for(String string5 : LIVING_ENTITIES) {
                typed = this.updateNamedChoice(typed, string5, DataConverterEntityUUID::updateLivingEntity);
            }

            for(String string6 : PROJECTILES) {
                typed = this.updateNamedChoice(typed, string6, DataConverterEntityUUID::updateProjectile);
            }
            ...
            return this.updateNamedChoice(typed, "minecraft:piglin", DataConverterEntityUUID::updatePiglin);
        });
    }

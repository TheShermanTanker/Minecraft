...
public class DataConverterVillage extends DataFix {
    public DataConverterVillage(Schema outputSchema, boolean changesType) {
    ...
    private static <T> Dynamic<T> updateChildren(Dynamic<T> dynamic) {
        return dynamic.asStreamOpt().map(DataConverterVillage::updateChildren).map(dynamic::createList).result().orElse(dynamic);
    }
    ...
    private static <T> Dynamic<T> updateCrop(Dynamic<T> dynamic, String cropId) {
        return dynamic.get(cropId).asNumber().result().isPresent() ? dynamic.set(cropId, DataConverterFlattenData.getTag(dynamic.get(cropId).asInt(0) << 4)) : dynamic;
    }
}

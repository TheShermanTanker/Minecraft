...
public class DataConverterSaddle extends DataConverterNamedEntity {
    public DataConverterSaddle(Schema outputSchema, boolean changesType) {
        super(outputSchema, changesType, "EntityHorseSaddleFix", DataConverterTypes.ENTITY, "EntityHorse");
    }
    ...
    protected Typed<?> fix(Typed<?> inputType) {
        ...
        if (!optional.isPresent() && dynamic.get("Saddle").asBoolean(false)) {
            ...
            typed = typed.set(opticFinder, Pair.of(DataConverterTypes.ITEM_NAME.typeName(), "minecraft:saddle"));

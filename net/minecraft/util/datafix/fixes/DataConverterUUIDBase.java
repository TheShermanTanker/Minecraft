...
public abstract class DataConverterUUIDBase extends DataFix {
    ...
    public DataConverterUUIDBase(Schema outputSchema, TypeReference typeReference) {
    ...
    protected static Optional<Dynamic<?>> replaceUUIDMLTag(Dynamic<?> dynamic, String oldKey, String newKey) {
        return dynamic.get(oldKey).result().flatMap(DataConverterUUIDBase::createUUIDFromML).map((dynamic2) -> {
            return dynamic.remove(oldKey).set(newKey, dynamic2);
        });
    }

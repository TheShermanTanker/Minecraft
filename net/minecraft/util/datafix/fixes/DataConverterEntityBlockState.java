...
public class DataConverterEntityBlockState extends DataFix {
    ...
    public DataConverterEntityBlockState(Schema outputSchema, boolean changesType) {
    ...
    public TypeRewriteRule makeRule() {
        ...
        Type<Pair<Either<Pair<String, Either<Integer, String>>, Unit>, Dynamic<?>>> type = DSL.and(DSL.optional(DSL.field("inTile", DSL.named(DataConverterTypes.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), DataConverterSchemaNamed.namespacedString())))), DSL.remainderType());
        Function<Typed<?>, Typed<?>> function3 = (typed) -> {
            return typed.update(type.finder(), DSL.remainderType(), Pair::getSecond);
        };
        return this.fixTypeEverywhereTyped("EntityBlockStateFix", schema.getType(DataConverterTypes.ENTITY), schema2.getType(DataConverterTypes.ENTITY), (typed) -> {
    ...
    private Typed<?> updateFallingBlock(Typed<?> typed) {
        Type<Either<Pair<String, Either<Integer, String>>, Unit>> type = DSL.optional(DSL.field("Block", DSL.named(DataConverterTypes.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), DataConverterSchemaNamed.namespacedString()))));
        Type<Either<Pair<String, Dynamic<?>>, Unit>> type2 = DSL.optional(DSL.field("BlockState", DSL.named(DataConverterTypes.BLOCK_STATE.typeName(), DSL.remainderType())));
        Dynamic<?> dynamic = typed.get(DSL.remainderFinder());
        return typed.update(type.finder(), type2, (either) -> {
            int i = either.map((pair) -> {
                return pair.getSecond().map((integer) -> {
                    return integer;
                }, DataConverterEntityBlockState::getBlockId);
            }, (unit) -> {
            ...
            return Either.left(Pair.of(DataConverterTypes.BLOCK_STATE.typeName(), DataConverterFlattenData.getTag(i << 4 | j)));
    ...
    private Typed<?> updateBlockToBlockState(Typed<?> typed, String oldIdKey, String oldDataKey, String newStateKey) {
        Type<Pair<String, Either<Integer, String>>> type = DSL.field(oldIdKey, DSL.named(DataConverterTypes.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), DataConverterSchemaNamed.namespacedString())));
        Type<Pair<String, Dynamic<?>>> type2 = DSL.field(newStateKey, DSL.named(DataConverterTypes.BLOCK_STATE.typeName(), DSL.remainderType()));
        ...
        return typed.update(type.finder(), type2, (pair) -> {
            ...
            return Pair.of(DataConverterTypes.BLOCK_STATE.typeName(), DataConverterFlattenData.getTag(i << 4 | j));
    ...
    private Typed<?> updateEntity(Typed<?> typed, String entityId, Function<Typed<?>, Typed<?>> function) {
        Type<?> type = this.getInputSchema().getChoiceType(DataConverterTypes.ENTITY, entityId);
        Type<?> type2 = this.getOutputSchema().getChoiceType(DataConverterTypes.ENTITY, entityId);

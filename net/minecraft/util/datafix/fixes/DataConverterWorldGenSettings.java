...
public class DataConverterWorldGenSettings extends DataFix {
    ...
    public DataConverterWorldGenSettings(Schema outputSchema, boolean changesType) {
        ...
        if (generatorOptions.isEmpty()) {
            ...
            if (i >= 0 && i <= 3) {
                ...
                stringBuilder.append(StreamSupport.stream(LAYER_SPLITTER.split(string2).spliterator(), false).map((stringx) -> {
                    ...
                    int m = i == 3 ? DataConverterEntityBlockState.getBlockId("minecraft:" + string4) : NumberUtils.toInt(string4, 0);
                    int n = l + 1;
                    int o = list2.size() > n ? NumberUtils.toInt(list2.get(n), 0) : 0;
                    return (j == 1 ? "" : j + "*") + DataConverterFlattenData.getTag(m << 4 | o).get("Name").asString("");

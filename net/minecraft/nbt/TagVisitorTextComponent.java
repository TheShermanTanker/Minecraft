...
public class TagVisitorTextComponent implements TagVisitor {
    ...
    public void visitList(NBTTagList element) {
        if (element.isEmpty()) {
            ...
            for(int i = 0; i < element.size(); ++i) {
                ...
                mutableComponent.addSibling((new TagVisitorTextComponent(this.indentation, this.depth)).visit(element.get(i)));
            }
            ...
            for(int j = 0; j < element.size(); ++j) {
                ...
                mutableComponent3.addSibling((new TagVisitorTextComponent(this.indentation, this.depth + 1)).visit(element.get(j)));
                if (j != element.size() - 1) {
    ...
    public void visitCompound(NBTTagCompound compound) {
        if (compound.isEmpty()) {
            ...
            for(Iterator<String> iterator = collection.iterator(); iterator.hasNext(); mutableComponent.addSibling(mutableComponent2)) {
                ...
                mutableComponent2 = (new ChatComponentText(Strings.repeat(this.indentation, this.depth + 1))).addSibling(handleEscapePretty(string)).append(NAME_VALUE_SEPARATOR).append(" ").addSibling((new TagVisitorTextComponent(this.indentation, this.depth + 1)).visit(compound.get(string)));
                if (iterator.hasNext()) {

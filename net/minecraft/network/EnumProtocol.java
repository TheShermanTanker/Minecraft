...
public enum EnumProtocol {
    ...
    private EnumProtocol(int id, EnumProtocol.ProtocolBuilder initializer) {
        this.id = id;
    ...
    @VisibleForDebug
    public Int2ObjectMap<Class<? extends Packet<?>>> getPacketsByIds(EnumProtocolDirection side) {
        Int2ObjectMap<Class<? extends Packet<?>>> int2ObjectMap = new Int2ObjectOpenHashMap<>();
        EnumProtocol.PacketSet<?> packetSet = this.flows.get(side);
        if (packetSet == null) {
    ...
    static class PacketSet<T extends PacketListener> {
        final Object2IntMap<Class<? extends Packet<T>>> classToId = SystemUtils.make(new Object2IntOpenHashMap<>(), (map) -> {
            map.defaultReturnValue(-1);
        });
        private final List<Function<PacketDataSerializer, ? extends Packet<T>>> idToDeserializer = Lists.newArrayList();

        public <P extends Packet<T>> EnumProtocol.PacketSet<T> addPacket(Class<P> type, Function<PacketDataSerializer, P> packetFactory) {

...
public enum EnumChatFormat {
    ...
    private static final Map<String, EnumChatFormat> FORMATTING_BY_NAME = Arrays.stream(values()).collect(Collectors.toMap((f) -> {
    ...
    private EnumChatFormat(String name, char code, boolean modifier) {
    ...
    @Nullable
    public static EnumChatFormat getByName(@Nullable String name) {
        return name == null ? null : FORMATTING_BY_NAME.get(cleanName(name));
    }

    @Nullable
    public static EnumChatFormat getById(int colorIndex) {
        if (colorIndex < 0) {
            return RESET;
        } else {
            for(EnumChatFormat chatFormatting : values()) {
    ...
    @Nullable
    public static EnumChatFormat getByCode(char code) {

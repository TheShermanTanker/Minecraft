...
public class HusbandryAdvancements implements Consumer<Consumer<Advancement>> {
    ...
    private Advancement.SerializedAdvancement addFood(Advancement.SerializedAdvancement task) {
        for(Item item : EDIBLE_ITEMS) {
            task.addCriterion(IRegistry.ITEM.getKey(item).getKey(), CriterionTriggerConsumeItem.TriggerInstance.usedItem(item));
        }
    ...
    private Advancement.SerializedAdvancement addBreedable(Advancement.SerializedAdvancement task) {
        for(EntityTypes<?> entityType : BREEDABLE_ANIMALS) {
            task.addCriterion(EntityTypes.getName(entityType).toString(), CriterionTriggerBredAnimals.TriggerInstance.bredAnimals(CriterionConditionEntity.Builder.entity().of(entityType)));
        }

        task.addCriterion(EntityTypes.getName(EntityTypes.TURTLE).toString(), CriterionTriggerBredAnimals.TriggerInstance.bredAnimals(CriterionConditionEntity.Builder.entity().of(EntityTypes.TURTLE).build(), CriterionConditionEntity.Builder.entity().of(EntityTypes.TURTLE).build(), CriterionConditionEntity.ANY));
        return task;
    }
    ...
    private Advancement.SerializedAdvancement addFishBuckets(Advancement.SerializedAdvancement task) {
        for(Item item : FISH_BUCKETS) {
            task.addCriterion(IRegistry.ITEM.getKey(item).getKey(), CriterionTriggerFilledBucket.TriggerInstance.filledBucket(CriterionConditionItem.Builder.item().of(item).build()));
        }
    ...
    private Advancement.SerializedAdvancement addFish(Advancement.SerializedAdvancement task) {
        for(Item item : FISH) {
            task.addCriterion(IRegistry.ITEM.getKey(item).getKey(), CriterionTriggerFishingRodHooked.TriggerInstance.fishedItem(CriterionConditionItem.ANY, CriterionConditionEntity.ANY, CriterionConditionItem.Builder.item().of(item).build()));
        }
    ...
    private Advancement.SerializedAdvancement addCatVariants(Advancement.SerializedAdvancement task) {
        EntityCat.TEXTURE_BY_TYPE.forEach((integer, resourceLocation) -> {
            task.addCriterion(resourceLocation.getKey(), CriterionTriggerTamedAnimal.TriggerInstance.tamedAnimal(CriterionConditionEntity.Builder.entity().of(resourceLocation).build()));
        });
        return task;
    }
}

...
public class ShapelessRecipeBuilder implements RecipeBuilder {
    ...
    public ShapelessRecipeBuilder(IMaterial output, int outputCount) {
        this.result = output.getItem();
        this.count = outputCount;
    }

    public static ShapelessRecipeBuilder shapeless(IMaterial output) {
        return new ShapelessRecipeBuilder(output, 1);
    }

    public static ShapelessRecipeBuilder shapeless(IMaterial output, int outputCount) {
    ...
    public ShapelessRecipeBuilder requires(IMaterial itemProvider) {
        return this.requires(itemProvider, 1);
    }

    public ShapelessRecipeBuilder requires(IMaterial itemProvider, int size) {
    ...
    public ShapelessRecipeBuilder requires(RecipeItemStack ingredient) {
        return this.requires(ingredient, 1);
    }

    public ShapelessRecipeBuilder requires(RecipeItemStack ingredient, int size) {
        for(int i = 0; i < size; ++i) {
    ...
    @Override
    public ShapelessRecipeBuilder unlockedBy(String string, CriterionInstance criterionTriggerInstance) {
    ...
    public void save(Consumer<FinishedRecipe> exporter, MinecraftKey recipeId) {
        ...
        exporter.accept(new ShapelessRecipeBuilder.Result(recipeId, this.result, this.count, this.group == null ? "" : this.group, this.ingredients, this.advancement, new MinecraftKey(recipeId.getNamespace(), "recipes/" + this.result.getItemCategory().getRecipeFolderName() + "/" + recipeId.getKey())));
    }
